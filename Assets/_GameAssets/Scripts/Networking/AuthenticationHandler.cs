...
public static class AuthenticationHandler
{
    ...
    public static async UniTask<AuthenticationState> DoAuth(int maxTries = 5)
    {
        ...
        {
            Debug.LogWarning("deniyor zaten!");
            await Authenticating();
            return AuthenticationState;
        }
        await SignInAnonymousslyAsync(maxTries);
        return AuthenticationState;
    }
    ...
    private static async UniTask SignInAnonymousslyAsync(int maxTries)
    {
        ...
        if (AuthenticationState != AuthenticationState.Authenticated)
        {
            Debug.LogWarning($"player was not sgined in successfully after {tries}tries.");
